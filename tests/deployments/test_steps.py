...
import nebula
import nebula.utilities.dockerutils
import pendulum
import pytest

from nebula_docker.deployments.steps import build_docker_image, push_docker_image
...
def mock_docker_client(monkeypatch):
    ...
    mock_docker_client_func = MagicMock(
        name="docker_client", spec=nebula.utilities.dockerutils.docker_client
    )
    mock_docker_client_func.return_value.__enter__.return_value = mock_client
    monkeypatch.setattr(
        "nebula_docker.deployments.steps.docker_client", mock_docker_client_func
    )
    return mock_client
...
def mock_pendulum(monkeypatch):
    ...
    monkeypatch.setattr("nebula_docker.deployments.steps.pendulum", mock_pendulum)
...
def test_build_docker_image(
    ...
    mock_docker_client.api.build.assert_called_once_with(
        ...
        decode=True,
        pull=True,
        labels=nebula.utilities.dockerutils.IMAGE_LABELS,
    )
...
def test_real_auto_dockerfile_build(docker_client_with_cleanup):
    ...
    try:
        ...
        cases = [
            {"command": "nebula version", "expected": nebula.__version__},
        ...
        docker_client_with_cleanup.containers.prune(
            filters={"label": "nebula-docker-test"}
        )
