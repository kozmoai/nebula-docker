...
from nebula.testing.fixtures import *  # noqa
from nebula.testing.utilities import nebula_test_harness
from nebula.utilities.dockerutils import IMAGE_LABELS, silence_docker_warnings

from nebula_docker.worker import CONTAINER_LABELS
...
def cleanup_all_new_docker_objects(docker: DockerClient, worker_id: str):
    IMAGE_LABELS["io.nebula.test-worker"] = worker_id
    CONTAINER_LABELS["io.nebula.test-worker"] = worker_id
    try:
        yield
    finally:
        for container in docker.containers.list(all=True):
            if container.labels.get("io.nebula.test-worker") == worker_id:
                _safe_remove_container(container)
            elif container.labels.get("io.nebula.delete-me"):
                _safe_remove_container(container)

        filters = {"label": f"io.nebula.test-worker={worker_id}"}
